...
def gmail_reply(to: str, subject: str, original_body: str) -> bool:
    reply = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages, CEO commands, and agent updates. Be concise and professional. Parse commands clearly, take action, report back briefly."}],
    max_tokens=400)
    if not reply:
        return False
...
def manager_listen_node(state: ManagerState) -> ManagerState:
    ...
    if wpp_status():
        try:
            url = f"{EVOLUTION_URL}/chat/findMessages/{EVOLUTION_INSTANCE}"
            resp = requests.post(url, json={"limit": 20}, headers=_evo_headers(), timeout=15)
            all_msgs = resp.json()
            records = all_msgs if isinstance(all_msgs, list) else all_msgs.get("messages", {}).get("records", [])
            unread = []
            for m in records:
                if not m.get("key", {}).get("fromMe", True):
                    ...
                    from_jid = m.get("key", {}).get("remoteJid", "")
                    if text:
                        unread.append({
                            "body": text,
                            "from": from_jid,
            ...
            log.error(f"[Manager] WhatsApp fetch error: {exc}")
...
def manager_handle_agent_msg_node(state: ManagerState) -> ManagerState:
    ...
    if msg_type == "oos_decision_request":
        ...
        decision  = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages, CEO commands, and agent updates. Be concise and professional. Parse commands clearly, take action, report back briefly."}], max_tokens=80)
        ...
        ceo_msg = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages, CEO commands, and agent updates. Be concise and professional. Parse commands clearly, take action, report back briefly."}], max_tokens=200)
        if ceo_msg:
...
def manager_handle_new_lead_node(state: ManagerState) -> ManagerState:
    ...
    for lead in leads:
        ...
        first_msg = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages, CEO commands, and agent updates. Be concise and professional. Parse commands clearly, take action, report back briefly."}], max_tokens=300)
...
def manager_handle_whatsapp_node(state: ManagerState) -> ManagerState:
    ...
    for msg in messages:
        ...
        if True:
            ...
            response = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages and CEO commands. Be concise, 2-3 lines max."},
                                                      {"role": "user", "content": f"CEO WhatsApp message: {body}\n\nParse and respond with action taken."}
                                ], max_tokens=150)
            if response:
            ...
            if rows:
                ...
                reply = call_llm([{"role": "system", "content": "You are the Manager of NoError AI automation agency. Handle client WhatsApp messages, CEO commands, and agent updates. Be concise and professional. Parse commands clearly, take action, report back briefly."}], max_tokens=350)
                if reply:
...
def analyzer_analyze_node(state: AnalyzerState) -> AnalyzerState:
    ...
    result = call_llm([{"role": "system", "content": "You are the Analyzer Agent for NoError agency. Analyze OOS posts, decide APPROACH or SKIP. Be brief and decisive."}], max_tokens=100)
...
def researcher_build_final_summary():
    ...
    summary = call_llm([{"role": "system", "content": "You are the Researcher agent for NoError agency. Build concise market intelligence reports. Bullet points only, actionable insights."}], max_tokens=1000)
...
# ============================================================
# WEBHOOK SERVER — Evolution WhatsApp webhook receiver
# ============================================================
...
async def whatsapp_webhook(request: Request):
    try:
        ...
        # Sirf incoming messages pe trigger
        if event in ("messages.upsert", "message.received"):
            msg_data = data.get("data", {})
            from_me = msg_data.get("key", {}).get("fromMe", True)
            if not from_me:
                db_notify("agent_channel", "manager")
                log.info("[Webhook] WhatsApp message received — manager notified")
        return {"status": "ok"}
    except Exception as exc:
...
def main():
    ...
    # Webhook server FIRST
    threading.Thread(target=start_webhook_server, daemon=True,
                     name="webhook-server").start()
    time.sleep(2)  # server ko start hone do
